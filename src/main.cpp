...
// coinvest, 구분자
const string strMessageMagic = "MamCoin Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // MamCoin: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
                        ...
                        bool* pfMissingInputs)
{
    ...
    // MamCoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
int64_t GetProofOfWorkReward(int64_t nFees) {
	...
	if (pindexBest->nHeight+1 > 0 && pindexBest->nHeight+1 <= 100) {
		nSubsidy = 1392740 * COIN;
	} else if (pindexBest->nHeight+1 > 100 && pindexBest->nHeight+1 <= 200) {
		nSubsidy = 696370 * COIN;
	} else if (pindexBest->nHeight+1 > 200 && pindexBest->nHeight+1 <= 300) {
		nSubsidy = 348185 * COIN;
	} else if (pindexBest->nHeight+1 > 300 && pindexBest->nHeight+1 <= 400) {
		nSubsidy = 174092.5 * COIN;
	} else if (pindexBest->nHeight+1 > 400 && pindexBest->nHeight+1 <= 500) {
		nSubsidy = 87046.25 * COIN;
	} else {
		nSubsidy = 0 * COIN;
	}
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees) {
    ...
    nSubsidy = nCoinAge * 33 / (365 * 33 + 8) * COIN_YEAR_REWARD;
...
// MamCoin: find last block index up to pindex
...
static unsigned int GetNextTargetRequired_(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // Includes MamCoin fix for wrong retargeting difficulty by Mammix2
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // MamCoin: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // MamCoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // MamCoin: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // MamCoin: track money supply and mint amount info
...
// MamCoin: total coin age spent in transaction, in the unit of coin-days.
...
// MamCoin: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProofOfStake)
{
    ...
    // MamCoin: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // MamCoin: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // MamCoin: record proof-of-stake hash value
    pindexNew->hashProofOfStake = hashProofOfStake;

    // MamCoin: compute stake modifier
...
bool CBlock::AcceptBlock()
{
    ...
    // MamCoin: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // MamCoin: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // MamCoin: getblocks may not obtain the ancestor block rejected
    ...
    // MamCoin: if responsible for sync-checkpoint send it
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "MamCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "1515646270 seconds since Jan 01 1970. (UTC)";
        CTransaction txNew;
		// coinvest, 최초 시작 시간
        txNew.nTime = 1515646270;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
		// coinvest, 최초 시작 시간
        block.nTime    = 1515646270;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
		// coinvest, nNonce
        block.nNonce   = 6596037;
        if(fTestNet)
        {
            block.nNonce   = 5600000;
        }
        ...
        //// debug print
		// coinvest, MerkleRoot
        assert(block.hashMerkleRoot == uint256("0x99d792b7df110fe59b7108cb07f48615f097139b8d0d9d691803c4f1711b8783"));
        block.print();
        ...
        // MamCoin: initialize synchronized checkpoint
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // MamCoin: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // MamCoin: tell downloading node about the latest block if it's
